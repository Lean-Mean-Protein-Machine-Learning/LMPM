...
It takes in a sequence (amino acid in 1 or 3 letter code), a localization
and an organism that the user provides. Returns the probability
that the protein is in the target localization for that organism.
"""
...
# using os.path.join to avoid issues across OS using different backslash
# symbols

# Importing dG parameters to params_dict
params = pd.read_csv(
    os.path.join(current_wd, "models", "Params.csv"), header=None
)
lst = params[0].values
...
# Load trained models
model_human = pickle.load(
    open(os.path.join(current_wd, "models", "human.pkl"), "rb")
)
model_yeast = pickle.load(
    open(os.path.join(current_wd, "models", "yeast.pkl"), "rb")
)
model_ecoli = pickle.load(
    open(os.path.join(current_wd, "models", "ecoli.pkl"), "rb")
)
model_human_dg = pickle.load(
    open(os.path.join(current_wd, "models", "human_dg.pkl"), "rb")
)
model_yeast_dg = pickle.load(
    open(os.path.join(current_wd, "models", "yeast_dg.pkl"), "rb")
)
model_ecoli_dg = pickle.load(
    open(os.path.join(current_wd, "models", "ecoli_dg.pkl"), "rb")
)
...
def model_organism_selection(organism, include_dg):
    ...
    if organism not in ["human", "yeast", "ecoli"]:
        raise ValueError(
            'The selected organism was: "'
            + str(organism)
            + '", but should be only human, yeast, ecoli, or all'
        )
    else:
        pass
...
def calculate_transmembrane_dg(seq):
    ...
    if len(seq) < 19:
        raise ValueError(
            "dG cannot be computed on sequences \
...
def predict_loc_simple(input_seq, organism, target_loc, include_dg=False):
    ...
    if organism not in ["human", "yeast", "ecoli"]:
        raise ValueError(
            'The selected organism was: "'
            + str(organism)
            + '", but should be only human, yeast or ecoli'
        )
    else:
        pass
    ...
    if target_loc not in ["secreted", "membrane", "cytoplasm"]:
        raise ValueError(
            'The selected probability was: "'
            + str(target_loc)
            + '", but should be either secreted, membrane or cytoplasm'
        )
    else:
        pass
...
def predict_location(
    ...
    if organism not in ["human", "yeast", "ecoli", "all"]:
        raise ValueError(
            'The selected organism was: "'
            + str(organism)
            + '", but should be only human, yeast, ecoli, or all (default)'
        )
    else:
        pass
    ...
    if target_loc not in ["secreted", "membrane", "cytoplasm", "all"]:
        raise ValueError(
            'The selected probability was: "'
            + str(target_loc)
            + '", but should be either secreted, membrane, cytoplasm or all'
            + ' (default)'
        )
    else:
        pass
    ...
    predicted_loc = pred_results.index[
        np.argmax(pred_results.loc[:, org_for_pred])
    ]
